...
import { FileText, CheckCircle, Clock, Award, Users, TrendingUp, Download, Eye, Edit, Save, Loader2 } from 'lucide-react';
...
const TeacherGradingPage: React.FC<TeacherGradingPageProps> = ({ teacherId }) => {
  ...
  const fetchAssignments = async () => {
    try {
      const response = await fetch(`http://localhost:5000/api/assignments/assignments/teacher/${teacherId}`);
  ...
  const fetchSubmissions = async (assignmentId: number) => {
    try {
      const [submissionsRes, analyticsRes] = await Promise.all([
        fetch(`http://localhost:5000/api/assignments/assignments/${assignmentId}/submissions`),
        fetch(`http://localhost:5000/api/assignments/analytics/assignment/${assignmentId}`)
      ]);
  ...
  const handleGrade = async () => {
    ...
    setGrading(true);
    try {
      const response = await fetch('http://localhost:5000/api/assignments/grades', {
        method: 'POST',
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-white to-blue-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        {!selectedAssignment ? (
          ...
          <div>
            <Button onClick={() => { setSelectedAssignment(null); setSubmissions([]); }} variant="outline" className="mb-6 font-bold">
              ← Subira
            </Button>
