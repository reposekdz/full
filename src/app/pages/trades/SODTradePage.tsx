...
const SODTradePage: React.FC<SODTradePageProps> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    const loadTradeData = async () => {
      try {
        ...
        // Keep page functional even if API is unavailable (do not use mock data)
        setTeachers([]);
        setStudents([]);
        setTradeInfo(null);
      }
  ...
  const stats = [
    { label: 'Active Students', value: tradeInfo?.total_students ?? students.length, icon: Users, color: 'from-blue-500 to-indigo-500' },
