...
async function updateAUTLevelsWorking() {
  let connection;
  
  try {
    console.log('🚀 Updating AUT trade levels with sub-levels...');
    
    connection = await mysql.createConnection(dbConfig);
    console.log('✅ Connected to database');
    
    // Check if trade_levels table exists, if not create a simple one
    try {
      ...
      `);
      console.log('✅ Ensured trade_levels table exists');
    } catch (error) {
      console.log('⚠️ Table creation issue:', error.message);
    }
    ...
    const autLevels = [
      ...
      { 
        ...
        level_number: 3, 
        sub_level: null,
        level_name: 'Certificate Level 3', 
      ...
      { 
        ...
        level_number: 4, 
        sub_level: 'A',
        level_name: 'Certificate Level 4A', 
      ...
      { 
        ...
        level_number: 4, 
        sub_level: 'B',
        level_name: 'Certificate Level 4B', 
    ...
    for (const level of autLevels) {
      await connection.execute(`
        INSERT INTO trade_levels (trade_code, level_number, sub_level, level_name, description, duration_months, prerequisites, is_active)
        VALUES (?, ?, ?, ?, ?, ?, ?, TRUE)
      `, [level.trade_code, level.level_number, level.sub_level, level.level_name, level.description, level.duration_months, level.prerequisites]);
    }
    ...
    const [levels] = await connection.execute(`
      SELECT trade_code, level_number, sub_level, level_name, description
      FROM trade_levels 
    ...
    levels.forEach(level => {
      ...
      console.log(`   Level ${levelDisplay}: ${level.level_name}`);
    ...
    const newApplications = [
      ...
      {
        application_number: 'APP2024009',
        first_name: 'Michael',
        last_name: 'Habimana',
        date_of_birth: '2002-01-15',
        gender: 'Male',
        phone: '+250788555666',
        email: 'michael.habimana@email.com',
        address: 'Kigali, Nyarugenge',
        parent_name: 'Christine Habimana',
        parent_phone: '+250788777888',
        previous_school: 'GS Kigali',
        trade_code: 'AUT',
        level_number: 3,
        sub_level: null,
        reason_for_applying: 'I want to start with basic automotive skills',
        status: 'approved'
      }
    ];
...
if (require.main === module) {
  updateAUTLevelsWorking()
    .then(() => {
      console.log('\n🎯 AUT levels update completed successfully!');
      process.exit(0);
    })
    .catch(() => process.exit(1));
}

module.exports = { updateAUTLevelsWorking };