  ...
  try {
    ...
    await connection.query(`
      ...
      SET first_name = COALESCE(first_name, 'User'),
          last_name = COALESCE(last_name, CAST(id AS CHAR))
      WHERE first_name IS NULL OR first_name = '' OR last_name IS NULL OR last_name = ''
    `);
    
    // Fix inventory
    await connection.query(`ALTER TABLE inventory_items ADD COLUMN IF NOT EXISTS is_active BOOLEAN DEFAULT TRUE`);
