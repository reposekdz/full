...
router.post('/schedule', async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      ...
      [exam_name, exam_type, academic_year, term, start_date, end_date, created_by]
    );
...
router.post('/sessions/:id/invigilators', async (req, res) => {
  try {
    ...
    const [session] = await db.query('SELECT exam_date, start_time, end_time FROM exam_sessions WHERE id = ?', [req.params.id]);
    ...
    for (const teacher_id of teacher_ids) {
      ...
      await db.query('INSERT INTO exam_invigilators (session_id, teacher_id) VALUES (?, ?)', [req.params.id, teacher_id]);
    }
...
router.get('/schedule/:id', async (req, res) => {
  try {
    const [schedule] = await db.query('SELECT * FROM exam_schedules WHERE id = ?', [req.params.id]);
...
router.put('/schedule/:id/publish', async (req, res) => {
  try {
    await db.query('UPDATE exam_schedules SET status = ?, published_at = NOW() WHERE id = ?', ['published', req.params.id]);
