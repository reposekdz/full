...
router.post('/admin/users', authenticateToken, requireRole(...ROLES.ADMIN), async (req, res) => {
  try {
    ...
    const [result] = await pool.execute(`
      INSERT INTO users (first_name, last_name, email, phone, role, password, is_active, created_at)
      VALUES (?, ?, ?, ?, ?, ?, 1, NOW())
    `, [first_name, last_name, email, phone, role, password || 'default123']);
...
router.get('/advisor/students', authenticateToken, requireRole(...ROLES.ADVISOR), async (req, res) => {
  try {
    ...
    let query = `
      SELECT DISTINCT s.*, t.trade_name,
