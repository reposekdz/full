...
async function setupApplicationsSystemFinal() {
  let connection;
  ...
  try {
    ...
    try {
      const [columns] = await connection.execute(`
        ...
        WHERE TABLE_SCHEMA = 'school_management' AND TABLE_NAME = 'trades'
      `);
      
      const columnNames = columns.map(col => col.COLUMN_NAME);
      console.log('✅ Found existing trades table with columns:', columnNames.join(', '));
      
      // Work with existing structure - just ensure our trades exist
      if (columnNames.includes('trade_code')) {
        // Use trade_code column
        await connection.execute(`
          INSERT IGNORE INTO trades (trade_code, trade_name, description, active) VALUES
        ...
        await connection.execute(`
          INSERT IGNORE INTO trades (code, name, description, active) VALUES
          ('AUT', 'Automotive Technology', 'Vehicle repair, maintenance, and automotive systems', TRUE),
          ('BDC', 'Building and Construction', 'Construction techniques, building design, and project management', TRUE),
          ('SOD', 'Software Development', 'Programming, web development, and software engineering', TRUE)
        `);
      ...
      console.log('⚠️ Could not update trades table:', error.message);
    }
    
    // Create trade_levels table for level management
    await connection.execute(`
    ...
    const [appDist] = await connection.execute(`
      SELECT trade_code, level_number, COUNT(*) as count
      FROM student_applications
    ...
    appDist.forEach(row => {
      console.log(`   ${row.trade_code} Level ${row.level_number}: ${row.count} applications`);
    });
...
if (require.main === module) {
  setupApplicationsSystemFinal()
    .then(() => {
      console.log('\n🎯 Setup completed successfully!');
      console.log('\n💡 Next Steps:');
      console.log('   1. Start your server: npm run dev');
      console.log('   2. Access the applications management UI');
      console.log('   3. Test the API endpoints');
      console.log('   4. Configure SMS notifications if needed');
      process.exit(0);
    })
    .catch((error) => {
      console.error('\n💥 Setup failed:', error);
...
module.exports = { setupApplicationsSystemFinal };