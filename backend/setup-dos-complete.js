...
async function runSetup() {
  ...
  try {
    ...
    console.log('[2/4] Inserting subjects...');
    ...
    for (const [code, name, type, desc, credits] of subjects) {
      await connection.query(
        'INSERT IGNORE INTO subjects (subject_code, subject_name, subject_type, description, credit_hours) VALUES (?,?,?,?,?)',
        [code, name, type, desc, credits]
      );
    }
    
    const [[{ total }]] = await connection.query('SELECT COUNT(*) as total FROM subjects');
    console.log(`✓ ${total} subjects inserted\n`);
    
    // Step 3: Assign to trades
    console.log('[3/4] Assigning subjects to trades...');
    ...
    for (const [trade, levels] of trades) {
      for (const level of levels) {
        ...
        const [generalSubjects] = await connection.query(
          "SELECT id, subject_code, subject_name FROM subjects WHERE subject_type='general_studies'"
        );
        
        for (const subject of generalSubjects) {
          await connection.query(
            'INSERT IGNORE INTO subject_trade_assignments (subject_id, subject_code, subject_name, trade_code, level_number, is_mandatory, academic_year) VALUES (?,?,?,?,?,?,?)',
            [subject.id, subject.subject_code, subject.subject_name, trade, level, true, '2025']
          );
          assignmentCount++;
        }
        ...
        const [tradeSubjects] = await connection.query(
          "SELECT id, subject_code, subject_name FROM subjects WHERE subject_code LIKE ?",
          [`${trade}%`]
        );
        
        for (const subject of tradeSubjects) {
          await connection.query(
            'INSERT IGNORE INTO subject_trade_assignments (subject_id, subject_code, subject_name, trade_code, level_number, is_mandatory, academic_year) VALUES (?,?,?,?,?,?,?)',
            [subject.id, subject.subject_code, subject.subject_name, trade, level, true, '2025']
          );
    ...
    console.log(`✓ ${assignments} assignments created\n`);
    ...
    const [tables] = await connection.query(`
      ...
      AND TABLE_NAME IN ('subjects', 'subject_trade_assignments', 'teacher_subject_assignments', 'class_subject_schedule', 'teacher_workload', 'dos_action_logs')
    `);
